...
try {
  ...
  // Check if Low Confidence sheet exists
  const sheetName = 'Needs Review';
  ...
  if (jsonData.length > 0) {
    ...
    if (confidenceKey) {
      ...
      const lowCount = jsonData.filter(row => row[confidenceKey] === 'Low').length;
      console.log(`Low confidence rows: ${lowCount}`);
    }
