...
import { IconGrid, IconBox, IconShoppingBag, IconFileText, IconSettings, IconLogOut, IconX } from "./ui";
...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Mobile Drawer (Slide-out on < lg) */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-[260px] bg-white shadow-2xl flex flex-col transition-transform duration-300 ease-in-out lg:hidden ${
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* Mobile Header with Logo and Close button */}
        <div className="flex items-center justify-between h-16 border-b border-gray-100 px-5 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <Logo className="w-8 h-8 rounded-lg flex-shrink-0" />
          ...
          {NAV.map(({ id, label, Icon }) => {
            ...
            return (
              <button
                ...
                className={`sidebar-nav-item w-full flex items-center gap-3.5 rounded-xl text-sm py-3 px-3 transition-all ${
                  active
        ...
        <div className="border-t border-gray-100 p-4 flex-shrink-0">
          <div className="flex items-center gap-3 px-2 py-2 rounded-xl bg-gray-50">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-gray-400 mt-1 truncate">admin@snehabazar.in</p>
            </div>
          </div>
        </div>
      </div>

      {/* Desktop Sidebar (lg and above: exactly preserved) */}
      <aside
        className="hidden lg:flex flex-col h-full bg-white border-r border-gray-100 flex-shrink-0 transition-all duration-300 overflow-hidden"
        style={{ width: w }}
      >
        {/* Logo */}
